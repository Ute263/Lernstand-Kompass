...
(function () {
  ...
  function makeQrSvg(text, options = {}) {
    ...
    return `
      <svg class="qr-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" width="${size * scale}" height="${size * scale}" role="img" aria-label="Tier-QR-Code">
  ...
  function setReserved(isFunction, x, y) {
    if (x >= 0 && y >= 0 && x < SIZE && y < SIZE) isFunction[y][x] = true;
  }
