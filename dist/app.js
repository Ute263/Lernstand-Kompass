...
const TEACHER_GROUPS = [
  ...
  },
  {
    ...
    sections: [
      ...
      ["workbookAssignments", "Material zuweisen"],
      ["workDone", "Was ist bearbeitet?"],
      ["workbookCatalog", "Deutsch & Mathe"],
      ["today", "Heute"],
...
function childWorkbookCatalogDetailLabel(item) {
  if (!item) return "";
  if (item.workbook === "ABC der Tiere 1") return item.topicTitle || item.title || abc1CatalogDisplayTitle(item);
...
async function updateChildWorkbookAssignmentStatus(assignmentId, status) {
  ...
  const nextStatus = {
    ...
    markedByChild: true,
    reviewStatus: assignment.autoConfirm === true ? "bestätigt" : "wartet",
  ...
  if (catalog && nextStatus.status === "fertig" && assignment.autoConfirm === true) {
    ...
    const linked = findWorkbookProgressDuplicate(nextState.entries || [], state.activeClassId, animal.id, catalog);
    nextState = {
      ...nextState,
      workbookAssignmentStatuses: (nextState.workbookAssignmentStatuses || []).map((item) => item.id === nextStatus.id ? {
        ...item,
        progressLinked: true,
...
async function confirmWorkbookAssignmentStatus(statusId, forcedStatus = "") {
  const statusRecord = (state.workbookAssignmentStatuses || []).find((item) => item.id === statusId);
  if (!statusRecord || statusRecord.reviewStatus !== "wartet") return;
  const catalog = workbookCatalogForClass(statusRecord.classId).find((item) => item.id === statusRecord.workbookCatalogId);
  const animal = state.animals.find((item) => item.id === statusRecord.animalId);
  if (!catalog || !animal) return;
  const timestamp = nowIso();
  const status = normalizeSimpleWorkStatus(forcedStatus || statusRecord.status);
  let nextState = upsertWorkbookProgressEntry(state, {
    classId: statusRecord.classId,
    animal,
    catalog,
    status,
    source: "Zuweisung",
    completedPages: status === "fertig" ? weeklyCatalogPages(catalog).map(String) : []
  });
  const linked = findWorkbookProgressDuplicate(nextState.entries || [], statusRecord.classId, animal.id, catalog);
  nextState = {
    ...nextState,
    workbookAssignmentStatuses: (nextState.workbookAssignmentStatuses || []).map((item) => item.id === statusId ? {
      ...item,
      status,
  ...
  globalMessage = "Zuweisungs-Markierung wurde bestätigt.";
  await persistAndRender(nextState);
}

async function rejectWorkbookAssignmentStatus(statusId) {
  const timestamp = nowIso();
  const statuses = (state.workbookAssignmentStatuses || []).map((item) => item.id === statusId ? {
    ...item,
    reviewStatus: "abgelehnt",
    updatedAt: timestamp
  } : item);
  globalMessage = "Zuweisungs-Markierung wurde abgelehnt.";
  await persistAndRender({ ...state, workbookAssignmentStatuses: statuses });
}
...
async function saveChildSelfReport(event) {
  ...
  const suggested = findWorkbookCatalogMatchForChildReport(subject, materialFamily, pageText, animal.id);
  const timestamp = nowIso();
  const report = {
    id: makeId(),
    classId: state.activeClassId,
    ...
    <section class="step-wrap">
      ...
      <div class="scanner-panel">
        <div class="qr-video-wrap">
          ...
          <div class="qr-scan-guide" aria-hidden="true"></div>
        </div>
        <canvas id="qrCanvas" class="qr-canvas"></canvas>
        <p class="message" id="scannerMessage">Kamera wird geöffnet...</p>
        <button class="secondary qr-retry-button" id="qrRetryButton" type="button" onclick="restartQrScanner()" hidden>Kamera neu starten</button>
      </div>
      <div class="qr-photo-fallback">
        <label class="secondary qr-photo-button" for="qrPhotoInput">QR-Code fotografieren</label>
        <input id="qrPhotoInput" class="qr-photo-input" type="file" accept="image/*" capture="environment" onchange="scanQrPhoto(event)">
        <small>Falls das Live-Scannen auf dem iPad nicht reagiert, kann der QR-Code direkt fotografiert werden.</small>
      </div>
...
function waitForQrVideo(video, timeoutMs = 5000) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = window.setTimeout(() => done(new Error("video-timeout")), timeoutMs);
...
async function startQrScanner() {
  ...
  const retry = document.querySelector("#qrRetryButton");
  if (!message || !video) return;
  try {
    if (!navigator.mediaDevices?.getUserMedia) throw new Error("no-camera");
    stopQrScanner();
    if (retry) retry.hidden = true;
    message.textContent = "Kamera wird geöffnet...";
    scannerStream = await requestQrCamera();
    if (screen !== "qrScanner") {
    ...
    if ("BarcodeDetector" in window) {
      try {
        const supported = typeof BarcodeDetector.getSupportedFormats === "function"
          ...
          : ["qr_code"];
        if (supported.includes("qr_code")) barcodeDetector = new BarcodeDetector({ formats: ["qr_code"] });
      } catch {
        barcodeDetector = null;
      }
    }
    ...
    message.textContent = "Halte den QR-Code vollständig in das helle Quadrat. Auf dem iPad darf er ruhig etwas näher an die Kamera.";
    scanQrFrame();
  } catch (error) {
    const name = String(error?.name || "");
    if (retry) retry.hidden = false;
    message.textContent = name === "NotAllowedError" || name === "SecurityError"
      ? "Die Kamera ist nicht freigegeben. Erlaube der Lernstand-Kompass-App bzw. dieser Webseite den Kamerazugriff und tippe dann auf „Kamera neu starten“."
      : "Die Kamera konnte nicht zuverlässig gestartet werden. Tippe auf „Kamera neu starten“ oder fotografiere den QR-Code direkt.";
  }
}
...
function decodeQrCanvas(canvas, sx, sy, sw, sh, targetWidth = 1000) {
  if (typeof window.jsQR !== "function" || !sw || !sh) return "";
  ...
  if (!context) return "";
  context.drawImage(document.querySelector("#qrVideo"), sx, sy, sw, sh, 0, 0, width, height);
...
function scanQrWithJsQr(video, canvas) {
  ...
  let token = decodeQrCanvas(canvas, 0, 0, vw, vh, 1100);
  if (token) return token;

  const cropScale = 0.78;
  const cw = Math.round(vw * cropScale);
  const ch = Math.round(vh * cropScale);
  const cx = Math.round((vw - cw) / 2);
  const cy = Math.round((vh - ch) / 2);
  token = decodeQrCanvas(canvas, cx, cy, cw, ch, 1100);
  return token || "";
}
...
async function scanQrPhoto(event) {
  ...
  if (typeof window.jsQR !== "function") {
    if (message) message.textContent = "Die QR-Erkennung konnte nicht geladen werden.";
    return;
  }
  try {
    if (message) message.textContent = "Foto wird geprüft …";
    ...
    if (!canvas) return;
    const width0 = img.width || img.naturalWidth;
    const height0 = img.height || img.naturalHeight;
    const scale = Math.min(1, 1400 / width0);
    ...
    if (result?.data) {
      if (message) message.textContent = "QR-Code erkannt …";
      await handleScannedQrToken(result.data.trim());
    } else if (message) {
      message.textContent = "Auf dem Foto wurde kein QR-Code erkannt. Bitte fotografiere die Karte gerade, vollständig und ohne starke Spiegelung.";
    }
  } catch (error) {
    console.warn("QR-Foto konnte nicht ausgewertet werden.", error);
    if (message) message.textContent = "Das Foto konnte nicht ausgewertet werden. Bitte versuche es noch einmal.";
  }
}
...
async function scanQrFrame() {
  ...
  try {
    ...
    if (barcodeDetector && video.readyState >= 2) {
      try {
        ...
        token = codes.find((code) => code?.rawValue)?.rawValue || "";
      } catch {
        barcodeDetector = null;
      }
    }
  ...
  scannerTimer = window.setTimeout(scanQrFrame, 140);
}
...
function renderTeacherSection(tab) {
  ...
  if (tab === "pendingReports") return renderPendingReportsDashboard();
  if (tab === "activeMaterials") return renderActiveWorkbookMaterialsPanel();
  if (tab === "workbookAssignments") return renderWorkbookAssignmentsDashboard();
  if (tab === "workDone") return renderWorkDoneControl();
  if (tab === "workbookCatalog") return `${renderWorkbookCatalogManager()}${renderGoalSettings()}`;
  if (tab === "childSettings") return `${renderChildViewSettingsPanel()}${renderProgressSettings()}`;
      ...
      <form class="settings-grid" onsubmit="saveChildViewSettings(event)">
        ...
        <label class="field">Mathe-Arbeitshefte anzeigen
          <select class="select-input" id="childSettingMinimaxVisibility">
            ...
            <option value="hidden" ${settings.minimaxVisibility === "hidden" ? "selected" : ""}>nein</option>
          </select>
        </label>
        <label class="check-row">
          <input type="checkbox" id="childSettingShowReports" ${settings.showSelfReports ? "checked" : ""}>
...
function renderPendingReportsDashboard() {
  const classId = getProgressClassId();
  return `
    ${renderPendingChildWorkbookReports(classId)}
    ${renderPendingWorkbookAssignmentStatuses(classId)}
    ${renderPendingWeeklyPlanStatuses(classId)}
  `;
}

function renderPendingWorkbookAssignmentStatuses(classId) {
  const rows = (state.workbookAssignmentStatuses || [])
    .filter((item) => item.classId === classId && item.markedByChild && item.reviewStatus === "wartet")
    .map((status) => {
      const assignment = (state.workbookAssignments || []).find((item) => item.id === status.assignmentId);
      const catalog = workbookCatalogForClass(classId).find((item) => item.id === status.workbookCatalogId);
      const animal = animalsForClass(classId).find((item) => item.id === status.animalId);
      return assignment && catalog && animal ? { status, assignment, catalog, animal } : null;
    })
    .filter(Boolean)
    .sort((a, b) => new Date(a.status.updatedAt || 0) - new Date(b.status.updatedAt || 0));
  return `
    <section class="panel">
      <h2>Markierungen aus Zuweisungen</h2>
      <p class="message">Diese Aufgaben wurden im Kinderbereich markiert und warten auf die Bestätigung der Lehrkraft.</p>
      <div class="table-scroll">
        <table>
          <thead><tr><th>Tier</th><th>Fach</th><th>Materialeintrag</th><th>Status</th><th>Aktion</th></tr></thead>
          <tbody>
            ${rows.map((row) => `
              <tr>
                <td>${teacherAnimalLabel(row.animal)}</td>
                <td>${escapeHtml(row.catalog.subject)}</td>
                <td>${escapeHtml(workbookCatalogFullLabel(row.catalog))}</td>
                <td>${simpleWorkStatusBadge(row.status.status)}</td>
                <td class="status-action-cell">
                  <button class="small-button" type="button" onclick="confirmWorkbookAssignmentStatus('${escapeAttribute(row.status.id)}')">bestätigen</button>
                  <button class="small-button" type="button" onclick="confirmWorkbookAssignmentStatus('${escapeAttribute(row.status.id)}','teilweise')">auf teilweise setzen</button>
                  <button class="danger small-button" type="button" onclick="rejectWorkbookAssignmentStatus('${escapeAttribute(row.status.id)}')">ablehnen</button>
                </td>
              </tr>
            `).join("") || `<tr><td colspan="5">Keine offenen Zuweisungs-Markierungen.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
function renderPendingWeeklyPlanStatuses(classId) {
  const rows = buildWeeklyProgressRows(classId)
    .filter((row) => row.statusRecord?.markedByChild && row.statusRecord.reviewStatus === "wartet" && normalizeSimpleWorkStatus(row.status) !== "offen" && !row.statusRecord.progressLinked);
  return `
    <section class="panel">
      <h2>Markierungen aus dem Wochenplan</h2>
      <p class="message">Wochenplan-Aufgaben werden erst nach der Bestätigung Teil des verbindlichen Lernstands.</p>
      <div class="table-scroll">
        <table>
          <thead><tr><th>Tier</th><th>Tag</th><th>Fach</th><th>Aufgabe</th><th>Status</th><th>Aktion</th></tr></thead>
          <tbody>
            ${rows.map((row) => `
              <tr>
                <td>${teacherAnimalLabel(row.animal)}</td>
                ...
                <td>${escapeHtml(row.pagesLabel)} · ${escapeHtml(row.topic || row.workbookLabel)}</td>
                <td>${simpleWorkStatusBadge(row.status)}</td>
                <td><button class="small-button" type="button" onclick="confirmWeeklyPlanProgress('${escapeAttribute(row.statusRecord.id)}')">bestätigen</button></td>
              </tr>
            `).join("") || `<tr><td colspan="6">Keine offenen Wochenplan-Markierungen.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
function renderWorkbookAssignmentsDashboard() {
  return renderWorkbookAssignmentManager(getProgressClassId());
}

function renderWorkDoneControl() {
  const classId = getProgressClassId();
  const entries = (state.entries || [])
    .filter((entry) => entry.classId === classId)
    .filter((entry) => !workDoneFilters.subject || entry.fach === workDoneFilters.subject)
    .filter((entry) => !workDoneFilters.status || normalizeSimpleWorkStatus(entry.workStatus || entry.status) === workDoneFilters.status)
    .filter((entry) => !workDoneFilters.source || String(entry.source || "").includes(workDoneFilters.source))
    .sort(sortNewest);
  return `
    <section class="panel">
      <h2>Was ist bearbeitet?</h2>
      <p class="message">Kontrollansicht über den zentralen Fortschritt. Jede Zeile zeigt Material, Status und Herkunft des Eintrags.</p>
      <div class="filters">
        <label class="field">Fach
          <select class="select-input" onchange="setWorkDoneFilter('subject', this.value)">
            <option value="">alles</option>
            ${SUBJECTS.map((subject) => `<option value="${subject}" ${workDoneFilters.subject === subject ? "selected" : ""}>${subject}</option>`).join("")}
          </select>
        </label>
        <label class="field">Status
          <select class="select-input" onchange="setWorkDoneFilter('status', this.value)">
            <option value="">alle</option>
            ${WEEKLY_PLAN_STATUSES.map((status) => `<option value="${status}" ${workDoneFilters.status === status ? "selected" : ""}>${status}</option>`).join("")}
          </select>
        </label>
        <label class="field">Quelle
          <select class="select-input" onchange="setWorkDoneFilter('source', this.value)">
            <option value="">alle</option>
            ${["Wochenplan", "Zuweisung", "Kind gemeldet", "Direkteingabe", "Korrektur"].map((source) => `<option value="${source}" ${workDoneFilters.source === source ? "selected" : ""}>${source}</option>`).join("")}
          </select>
        </label>
      </div>
      <div class="table-scroll">
        <table>
          <thead><tr><th>Tier</th><th>Fach</th><th>Material</th><th>Bereich</th><th>Seite</th><th>Thema</th><th>Status</th><th>Quelle</th><th>zuletzt geändert</th></tr></thead>
          <tbody>
            ${entries.map((entry) => `
              <tr>
                ...
                <td>${escapeHtml(entry.workbookPart || entry.workbookArea || "–")}</td>
                <td>${escapeHtml(entry.pages || entryWorkLabel(entry))}</td>
                <td>${escapeHtml(progressEntryTopicLabel(entry))}</td>
                <td>${simpleWorkStatusBadge(entry.workStatus || entry.status)}</td>
                <td>${escapeHtml(entry.source || "Direkteingabe")}</td>
                <td>${formatDateTime(entry.updatedAt || entry.datumUhrzeit)}</td>
              </tr>
            `).join("") || `<tr><td colspan="9">Für diese Auswahl gibt es keine Einträge.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
function setWorkDoneFilter(field, value) {
  workDoneFilters = { ...workDoneFilters, [field]: value };
  render();
}

function applyAnimalGroupSelection(groupId, target) {
  const group = (state.animalGroups || []).find((item) => item.id === groupId);
  if (!group) return;
  const selector = target === "weeklyAnimalCheckbox"
    ? ".weeklyAnimalCheckbox"
    : `input[name='${target}']`;
  document.querySelectorAll(selector).forEach((checkbox) => {
    checkbox.checked = group.animalIds.includes(checkbox.value);
  });
  const selectedMode = document.querySelector("input[name='weeklyAssignmentMode'][value='selected']");
  if (target === "weeklyAnimalCheckbox" && selectedMode) selectedMode.checked = true;
  const assignmentMode = document.querySelector("#assignmentMode");
  if (target === "assignmentAnimal" && assignmentMode) assignmentMode.value = "selected";
}
  ...
  return `
    <tr>
      ...
      <td>
        ${options.length ? `
          ${renderTeacherMaterialPickerControl({
            targetId: `childReportCatalog_${report.id}`,
            subject: report.subject,
            selectedId: report.selectedWorkbookCatalogId || report.suggestedWorkbookCatalogId || "",
            label: "Zuordnung",
            classId: report.classId || state.activeClassId,
            allowUnselected: !(report.selectedWorkbookCatalogId || report.suggestedWorkbookCatalogId)
          })}
      ...
      <td class="status-action-cell">
        ...
        <button class="small-button" type="button" onclick="editChildWorkbookReport('${escapeAttribute(report.id)}')">ändern</button>
        ...
        <button class="small-button" type="button" onclick="confirmChildWorkbookReport('${escapeAttribute(report.id)}','fertig','catalog')">gesamten Bereich fertig</button>
...
function renderAnimalProgressOverview(classId, animal) {
  ...
  const weeklyCounts = WEEKLY_PLAN_STATUSES.reduce((counts, status) => ({
    ...counts,
    [status]: weeklyRows.filter((row) => normalizeSimpleWorkStatus(row.status) === status).length
  }), {});
  const pendingCount = (state.childWorkbookReports || []).filter((item) => item.classId === classId && item.animalId === animal.id && item.reviewStatus === "wartet").length
    + (state.workbookAssignmentStatuses || []).filter((item) => item.classId === classId && item.animalId === animal.id && item.reviewStatus === "wartet").length;
  ...
  return `
    <section class="panel">
      ...
      <div class="summary-grid">
        ...
        <div>Diese Woche</div><strong>${weeklyRows.length ? `${weeklyCounts.offen || 0} offen · ${weeklyCounts.teilweise || 0} teilweise · ${weeklyCounts.fertig || 0} fertig` : "kein aktueller Wochenplan"}</strong>
        <div>Von Kindern gemeldet</div><strong>${pendingCount ? `${pendingCount} warten auf Bestätigung` : "keine offenen Meldungen"}</strong>
        <div>Aktives Deutsch-Material</div><strong>${escapeHtml(activeWorkbookSettingLabel(activeWorkbookSetting(classId, "Deutsch", { animalId: animal.id })))}</strong>
        <div>Aktives Mathe-Material</div><strong>${escapeHtml(activeWorkbookSettingLabel(activeWorkbookSetting(classId, "Mathe", { animalId: animal.id })))}</strong>
      </div>
    </section>
    <section class="panel">
      <h2>Details</h2>
      ...
      <div class="table-scroll">
        <table>
          <thead><tr><th>Fach</th><th>Material</th><th>Bereich</th><th>Seite</th><th>Thema</th><th>Status</th><th>Quelle</th><th>zuletzt geändert</th></tr></thead>
          <tbody>
            ...
            `).join("") || `<tr><td colspan="8">Noch keine Fortschrittseinträge vorhanden.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
function renderTeacherMaterialPickerControl({ targetId, subject = "", selectedId = "", label = "Material auswählen", classId = "", allowUnselected = false, animalId = "", groupId = "" }) {
  ...
  const defaultId = subject ? defaultWorkbookCatalogIdForSubject(subject, { animalId, groupId, classId: classId || state.activeClassId }) : "";
  const selected = catalog.find((item) => item.id === selectedId)
    || catalog.find((item) => item.id === defaultId)
    || (allowUnselected ? null : catalog[0])
    || null;
  return `
    <div class="field material-picker-control">
      <strong>${escapeHtml(label)}</strong>
      <input type="hidden" id="${escapeAttribute(targetId)}" value="${escapeAttribute(selected?.id || "")}">
      <div class="material-picker-current" id="${escapeAttribute(targetId)}Display">${selected ? escapeHtml(workbookCatalogFullLabel(selected)) : "noch keine Auswahl"}</div>
      <button class="secondary" type="button" onclick="openTeacherMaterialPicker('field', ${jsString(subject)}, ${jsString(targetId)}, document.getElementById(${jsString(targetId)})?.value || '', ${jsString(classId || state.activeClassId)}, { animalId: ${jsString(animalId)}, groupId: ${jsString(groupId)} })">Anderes Material wählen</button>
    </div>
  `;
}
...
function openTeacherMaterialPicker(mode, subject = "", targetId = "", selectedId = "", classId = "", meta = {}) {
  ...
  teacherMaterialPicker = normalizeTeacherMaterialPicker({
    ...
    topic: selected ? materialPickerTopic(selected) : "",
    itemId: selected?.id || "",
    meta
  });
  appendTeacherMaterialPicker();
...
function sortWorkbookCatalogItems(a, b) {
...
function schoolYearSortValue(value) {
  ...
  if (["1", "2", "3", "4"].includes(normalized)) return Number(normalized);
  if (normalized === "cross") return 5;
  if (normalized === "none") return 6;
  return 9;
}

function materialPickerSection(item) {
  if (!item) return "";
  if (item.workbook === "ABC der Tiere 1") {
    return item.part || item.category || item.area || "Arbeitsheft";
  }
  if (item.workbook === "ABC der Tiere 2") {
    return item.part || "Arbeitsheft";
  }
  if (String(item.workbook || "").startsWith("MiniMax")) {
    return item.part || "Gesamtes Heft";
  }
  return item.part || item.category || "Allgemein";
}

function materialPickerTopic(item) {
  if (!item) return "";
  if (item.workbook === "ABC der Tiere 1") {
    return abc1CatalogDisplayTitle(item) || item.title || item.area || item.category || "Eintrag";
  }
  if (item.workbook === "ABC der Tiere 2") {
    return item.area || item.title || "Eintrag";
  }
  if (String(item.workbook || "").startsWith("MiniMax")) {
    return item.area || item.title || "Eintrag";
  }
  return item.area || item.title || item.category || "Eintrag";
}

function workbookCatalogPickerPageLabel(item, currentTopic = "") {
  if (!item) return "";
  const pageLabel = pageRangeLabel(item);
  if (item.workbook === "ABC der Tiere 1") {
    const title = abc1CatalogDisplayTitle(item);
    const detail = abc1CatalogFibelLabel(item);
    return [pageLabel, title && title !== currentTopic ? title : "", detail].filter(Boolean).join(" · ");
  }
  return `${pageLabel}${item.title && item.title !== currentTopic ? ` · ${item.title}` : ""}`;
}

function workbookCatalogPickerSummary(item) {
  if (!item) return "";
  if (item.workbook === "ABC der Tiere 1") {
    return [
      schoolYearLabel(item.schoolYear),
      item.subject,
      item.workbook,
      abc1CatalogBookLine(item),
      abc1CatalogDisplayTitle(item),
      pageRangeLabel(item),
      abc1CatalogFibelLabel(item)
    ].filter(Boolean).join(" → ");
  }
  return [
    schoolYearLabel(item.schoolYear),
    item.subject,
    item.workbook,
    materialPickerSection(item),
    materialPickerTopic(item),
    pageRangeLabel(item)
  ].filter(Boolean).join(" → ");
}
...
function normalizeTeacherMaterialPicker(request) {
  const normalizedRequest = {
    ...request,
    schoolYear: normalizeSchoolYear(request.schoolYear || effectiveActiveSchoolYear(request.classId || state.activeClassId, request.subject || "Deutsch", request.meta || {}))
  };
  let catalog = teacherMaterialPickerCatalog(normalizedRequest);
  if (!catalog.length && !normalizedRequest.showAllYears) {
    normalizedRequest.showAllYears = true;
    catalog = teacherMaterialPickerCatalog(normalizedRequest);
  }
  const workbooks = uniqueSorted(catalog.map((item) => item.workbook));
  const workbook = workbooks.includes(normalizedRequest.workbook) ? normalizedRequest.workbook : workbooks[0] || "";
  const workbookItems = catalog.filter((item) => item.workbook === workbook);
  const sections = uniqueInOrder(workbookItems.map(materialPickerSection));
  const section = sections.includes(normalizedRequest.section) ? normalizedRequest.section : sections[0] || "";
  const sectionItems = workbookItems.filter((item) => materialPickerSection(item) === section);
  const topics = uniqueInOrder(sectionItems.map(materialPickerTopic));
  const topic = topics.includes(normalizedRequest.topic) ? normalizedRequest.topic : topics[0] || "";
  const topicItems = sectionItems.filter((item) => materialPickerTopic(item) === topic);
  const itemId = topicItems.some((item) => item.id === normalizedRequest.itemId) ? normalizedRequest.itemId : topicItems[0]?.id || "";
  return { ...normalizedRequest, workbook, section, topic, itemId };
}

function uniqueSorted(values) {
  return [...new Set(values.filter(Boolean))].sort((a, b) => String(a).localeCompare(String(b), "de", { numeric: true }));
}
...
function updateTeacherMaterialPicker(field, value) {
  if (!teacherMaterialPicker) return;
  const next = { ...teacherMaterialPicker, [field]: value };
  if (field === "subject") {
    next.workbook = "";
    next.section = "";
    next.topic = "";
    next.itemId = "";
    next.schoolYear = effectiveActiveSchoolYear(next.classId || state.activeClassId, value, next.meta || {});
    next.showAllYears = false;
  ...
  teacherMaterialPicker = normalizeTeacherMaterialPicker(next);
  appendTeacherMaterialPicker();
}

function appendTeacherMaterialPicker() {
  document.querySelector("#teacherMaterialPickerOverlay")?.remove();
  if (!teacherMaterialPicker || screen !== "teacher") return;
  app.insertAdjacentHTML("beforeend", renderTeacherMaterialPicker());
}

function renderTeacherMaterialPicker() {
  ...
  const schoolYears = uniqueInOrder([
    request.schoolYear,
    ...allCatalog.map((item) => item.schoolYear || "none")
  ].filter(Boolean));
  const workbooks = uniqueSorted(catalog.map((item) => item.workbook));
  const workbookItems = catalog.filter((item) => item.workbook === request.workbook);
  const sections = uniqueInOrder(workbookItems.map(materialPickerSection));
  const sectionItems = workbookItems.filter((item) => materialPickerSection(item) === request.section);
  const topics = uniqueInOrder(sectionItems.map(materialPickerTopic));
      ...
      <section class="training-modal-card teacher-material-picker-card">
        ...
        ${selected ? `
          <div class="material-picker-summary">
            ...
            <span>${escapeHtml(workbookCatalogPickerSummary(selected))}</span>
          </div>
        ` : `<div class="empty">Für diese Auswahl ist noch kein Katalogeintrag hinterlegt.</div>`}
        <div class="confirm-actions">
          <button class="primary" type="button" onclick="applyTeacherMaterialPickerSelection()" ${selected ? "" : "disabled"}>Übernehmen</button>
          <button class="secondary" type="button" onclick="closeTeacherMaterialPicker()">Abbrechen</button>
        </div>
      </section>
    </div>
  `;
...
function applyTeacherMaterialPickerSelection() {
  ...
  const input = document.getElementById(request.targetId);
  const display = document.getElementById(`${request.targetId}Display`);
  if (input) input.value = selected.id;
  if (display) display.textContent = workbookCatalogFullLabel(selected);
  closeTeacherMaterialPicker();
}

function closeTeacherMaterialPicker() {
  teacherMaterialPicker = null;
  document.querySelector("#teacherMaterialPickerOverlay")?.remove();
}
...
function renderWorkbookAssignmentManager(classId) {
  const animals = animalsForClass(classId).filter((animal) => animal.aktiv);
  const groups = (state.animalGroups || []).filter((group) => group.classId === classId);
  ...
  const defaultAssignmentId = defaultWorkbookCatalogIdForSubject("Deutsch", { classId }) || defaultWorkbookCatalogIdForSubject("Mathe", { classId }) || catalog[0]?.id || "";
  return `
    <section class="panel">
      ...
      <form class="inline-form" onsubmit="saveWorkbookAssignment(event)">
        ${renderTeacherMaterialPickerControl({
          targetId: "assignmentCatalogId",
          selectedId: defaultAssignmentId,
        ...
        ${groups.length ? `
          <label class="field">Tiergruppe als Auswahlhilfe
            <select class="select-input" onchange="applyAnimalGroupSelection(this.value, 'assignmentAnimal')">
...
function renderActiveWorkbookMaterialsPanel() {
  const classId = getProgressClassId();
  const animals = animalsForClass(classId).filter((animal) => animal.aktiv);
  const groups = (state.animalGroups || []).filter((group) => group.classId === classId);
  const settings = activeWorkbookMaterialsForClass(classId);
  const classSchoolYear = activeClassSchoolYear(classId);
  return `
    <section class="panel">
      <h2>Aktives Arbeitsmaterial</h2>
      <p class="message">Lege hier fest, welches Material im Alltag zuerst vorgeschlagen wird. Das ist eine Voreinstellung, keine Sperre: Über „Anderes Material wählen“ bleibt die vollständige Auswahl erreichbar.</p>
      <form class="inline-form active-school-year-form" onsubmit="saveActiveClassSchoolYear(event)">
        <label class="field">Aktives Schuljahr der Klasse
          <select class="select-input" id="activeClassSchoolYear">
            ${SCHOOL_YEAR_OPTIONS.filter(([value]) => ["1", "2", "3", "4"].includes(value)).map(([value, label]) => `<option value="${escapeAttribute(value)}" ${classSchoolYear === value ? "selected" : ""}>${escapeHtml(label)}</option>`).join("")}
          </select>
        </label>
        <button class="primary" type="submit">Schuljahr speichern</button>
      </form>
      <div class="active-material-grid">
        <form class="active-material-card" onsubmit="saveActiveWorkbookMaterial(event, 'class', 'Deutsch', '', '')">
          <h3>Klassenstandard Deutsch</h3>
          ${renderActiveWorkbookMaterialSelects("class", "Deutsch", "", "")}
          <button class="primary" type="submit">Deutsch-Standard speichern</button>
        </form>
        <form class="active-material-card" onsubmit="saveActiveWorkbookMaterial(event, 'class', 'Mathe', '', '')">
          <h3>Klassenstandard Mathe</h3>
          ${renderActiveWorkbookMaterialSelects("class", "Mathe", "", "")}
          <button class="primary" type="submit">Mathe-Standard speichern</button>
        </form>
      </div>
    </section>

    <section class="panel">
      <h2>Gruppenabweichungen</h2>
      ${groups.length ? groups.map((group) => `
        <article class="active-material-row">
          <strong>${escapeHtml(group.name)}</strong>
          <form class="inline-form" onsubmit="saveActiveWorkbookMaterial(event, 'group', 'Deutsch', '', '${escapeAttribute(group.id)}')">
            ${renderActiveWorkbookMaterialSelects("group", "Deutsch", "", group.id)}
            <button class="small-button" type="submit">Deutsch speichern</button>
            <button class="secondary small-button" type="button" onclick="clearActiveWorkbookMaterial('group','Deutsch','', '${escapeAttribute(group.id)}')">Klassenstandard nutzen</button>
          </form>
          <form class="inline-form" onsubmit="saveActiveWorkbookMaterial(event, 'group', 'Mathe', '', '${escapeAttribute(group.id)}')">
            ${renderActiveWorkbookMaterialSelects("group", "Mathe", "", group.id)}
            <button class="small-button" type="submit">Mathe speichern</button>
            <button class="secondary small-button" type="button" onclick="clearActiveWorkbookMaterial('group','Mathe','', '${escapeAttribute(group.id)}')">Klassenstandard nutzen</button>
          </form>
        </article>
      `).join("") : `<div class="empty">Noch keine Gruppen angelegt.</div>`}
    </section>

    <section class="panel">
      <h2>Einzelkind-Abweichungen</h2>
      <p class="message">Einzelkind-Abweichungen überschreiben Gruppen und Klassenstandard.</p>
      <div class="table-scroll">
        <table>
          <thead><tr><th>Tier</th><th>Deutsch</th><th>Mathe</th></tr></thead>
          <tbody>
            ${animals.map((animal) => `
              <tr>
                <td><strong>${teacherAnimalLabel(animal)}</strong></td>
                <td>
                  ${renderActiveMaterialEffectiveSummary(animal, "Deutsch")}
                  <form class="inline-form compact-active-material-form" onsubmit="saveActiveWorkbookMaterial(event, 'animal', 'Deutsch', '${escapeAttribute(animal.id)}', '')">
                    ${renderActiveWorkbookMaterialSelects("animal", "Deutsch", animal.id, "")}
                    <button class="small-button" type="submit">speichern</button>
                    <button class="secondary small-button" type="button" onclick="clearActiveWorkbookMaterial('animal','Deutsch','${escapeAttribute(animal.id)}','')">Standard nutzen</button>
                  </form>
                </td>
                <td>
                  ${renderActiveMaterialEffectiveSummary(animal, "Mathe")}
                  <form class="inline-form compact-active-material-form" onsubmit="saveActiveWorkbookMaterial(event, 'animal', 'Mathe', '${escapeAttribute(animal.id)}', '')">
                    ${renderActiveWorkbookMaterialSelects("animal", "Mathe", animal.id, "")}
                    <button class="small-button" type="submit">speichern</button>
                    <button class="secondary small-button" type="button" onclick="clearActiveWorkbookMaterial('animal','Mathe','${escapeAttribute(animal.id)}','')">Standard nutzen</button>
                  </form>
                </td>
              </tr>
            `).join("") || `<tr><td colspan="3">Noch keine aktiven Tiere vorhanden.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
function activeWorkbookMaterialsForClass(classId) {
  const today = formatFileDate(new Date());
  return (state.activeWorkbookMaterials || [])
    .filter((item) => item.classId === classId && item.active !== false)
    .filter((item) => (!item.validFrom || item.validFrom <= today) && (!item.validTo || item.validTo >= today));
}

function activeClassSchoolYear(classId = state.activeClassId) {
  const classItem = state.classes.find((item) => item.id === classId);
  return normalizeSchoolYear(classItem?.activeSchoolYear || inferSchoolYearFromClassName(classItem?.name || ""));
}
...
function schoolYearLabel(value) {
  const normalized = normalizeSchoolYear(value);
  return SCHOOL_YEAR_OPTIONS.find(([option]) => option === normalized)?.[1] || (normalized ? `Schuljahr ${normalized}` : "ohne Schuljahr");
}

function schoolYearsForSelect(values) {
  const normalized = values.map(normalizeSchoolYear).filter(Boolean);
  const defaults = ["1", "2", "3", "4"];
  return uniqueInOrder([...normalized, ...defaults]).filter(Boolean);
}

function materialMatchesSchoolYear(item, schoolYear) {
  const selected = normalizeSchoolYear(schoolYear);
  if (!selected) return true;
  const itemYear = normalizeSchoolYear(item.schoolYear || "none");
  return itemYear === selected || itemYear === "cross";
}

function activeWorkbookSetting(classId, subject, { animalId = "", groupId = "" } = {}) {
  ...
  if (groupId) {
    const groupSetting = settings.find((item) => item.scope === "group" && item.groupId === groupId);
    if (groupSetting) return groupSetting;
  }
...
function activeWorkbookCatalogCandidates(subject, { animalId = "", classId = state.activeClassId, groupId = "" } = {}) {
  ...
  const items = workbookCatalogForClass(classId)
    ...
    .filter((item) => materialMatchesSchoolYear(item, schoolYear));
  const fallbackItems = workbookCatalogForClass(classId).filter((item) => item.active !== false && item.subject === subject);
  const scopedItems = items.length ? items : fallbackItems;
  if (!setting?.workbook) return scopedItems;
  const filtered = scopedItems.filter((item) => (
    item.workbook === setting.workbook
    && (!setting.section || materialPickerSection(item) === setting.section)
  ));
  return filtered.length ? filtered : scopedItems;
}

function defaultWorkbookCatalogIdForSubject(subject, { animalId = "", classId = state.activeClassId, groupId = "" } = {}) {
  return activeWorkbookCatalogCandidates(subject, { animalId, classId, groupId }).sort(sortWorkbookCatalogItems)[0]?.id || "";
}

function activeWorkbookCatalogIdSet(subject, { animalId = "", classId = state.activeClassId, groupId = "" } = {}) {
  return new Set(activeWorkbookCatalogCandidates(subject, { animalId, classId, groupId }).map((item) => item.id));
}

function activeWorkbookSettingLabel(setting) {
  if (!setting?.workbook) return "kein aktives Material festgelegt";
  return [schoolYearLabel(setting.schoolYear || inferSchoolYearFromWorkbook(setting.workbook)), setting.workbook, setting.section].filter(Boolean).join(" · ");
}

function activeWorkbookSettingScopeLabel(setting) {
  if (!setting) return "kein Klassenstandard";
  if (setting.scope === "animal") return "abweichend vom Klassenstandard";
  if (setting.scope === "group") return "Gruppenstandard";
  return "Klassenstandard";
}

function renderActiveMaterialEffectiveSummary(animal, subject) {
  const setting = activeWorkbookSetting(animal.classId || state.activeClassId, subject, { animalId: animal.id });
  return `
    <div class="active-material-summary">
      <strong>${escapeHtml(activeWorkbookSettingLabel(setting))}</strong>
      <span>${escapeHtml(activeWorkbookSettingScopeLabel(setting))}</span>
    </div>
  `;
}

function renderActiveWorkbookMaterialSelects(scope, subject, animalId = "", groupId = "") {
  const classId = getProgressClassId();
  const setting = activeWorkbookSetting(classId, subject, { animalId, groupId });
  const selectedSchoolYear = normalizeSchoolYear(setting?.schoolYear || activeClassSchoolYear(classId));
  const catalog = workbookCatalogForClass(classId)
    .filter((item) => item.active !== false && item.subject === subject)
    .filter((item) => materialMatchesSchoolYear(item, selectedSchoolYear));
  const fallbackCatalog = workbookCatalogForClass(classId).filter((item) => item.active !== false && item.subject === subject);
  const scopedCatalog = catalog.length ? catalog : fallbackCatalog;
  const workbooks = uniqueSorted(scopedCatalog.map((item) => item.workbook));
  const selectedWorkbook = setting?.workbook && workbooks.includes(setting.workbook) ? setting.workbook : workbooks[0] || "";
  const sections = uniqueInOrder(scopedCatalog.filter((item) => item.workbook === selectedWorkbook).map(materialPickerSection));
  const selectedSection = setting?.section && sections.includes(setting.section) ? setting.section : sections[0] || "";
  const key = activeWorkbookFormKey(scope, subject, animalId, groupId);
  return `
    <input type="hidden" name="subject" value="${escapeAttribute(subject)}">
    <label class="field compact-field">Schuljahr
      <select class="select-input" id="${escapeAttribute(key)}SchoolYear" onchange="syncActiveWorkbookWorkbooks('${escapeAttribute(key)}', '${escapeAttribute(subject)}')">
        ${SCHOOL_YEAR_OPTIONS.map(([value, label]) => `<option value="${escapeAttribute(value)}" ${selectedSchoolYear === value ? "selected" : ""}>${escapeHtml(label)}</option>`).join("")}
      </select>
    </label>
    <label class="field compact-field">Material
      <select class="select-input" id="${escapeAttribute(key)}Workbook" onchange="syncActiveWorkbookSections('${escapeAttribute(key)}', '${escapeAttribute(subject)}')">
        ${workbooks.map((workbook) => `<option value="${escapeAttribute(workbook)}" ${selectedWorkbook === workbook ? "selected" : ""}>${escapeHtml(workbook)}</option>`).join("")}
      </select>
    </label>
    <label class="field compact-field">Teil / Bereich
      <select class="select-input" id="${escapeAttribute(key)}Section" data-selected="${escapeAttribute(selectedSection)}">
        ${sections.map((section) => `<option value="${escapeAttribute(section)}" ${selectedSection === section ? "selected" : ""}>${escapeHtml(section)}</option>`).join("")}
      </select>
    </label>
    <label class="field compact-field">gültig ab optional
      <input class="text-input" type="date" id="${escapeAttribute(key)}From" value="${escapeAttribute(setting?.validFrom || "")}">
    </label>
    <label class="field compact-field">gültig bis optional
      <input class="text-input" type="date" id="${escapeAttribute(key)}To" value="${escapeAttribute(setting?.validTo || "")}">
    </label>
  `;
}

function activeWorkbookFormKey(scope, subject, animalId = "", groupId = "") {
  return `activeMat_${scope}_${subject}_${animalId || groupId || "class"}`.replace(/[^a-zA-Z0-9_]/g, "_");
}

function syncActiveWorkbookWorkbooks(key, subject) {
  const schoolYear = document.getElementById(`${key}SchoolYear`)?.value || "";
  const workbookSelect = document.getElementById(`${key}Workbook`);
  if (!workbookSelect) return;
  const catalog = workbookCatalogForClass(getProgressClassId())
    .filter((item) => item.active !== false && item.subject === subject)
    .filter((item) => materialMatchesSchoolYear(item, schoolYear));
  const fallbackCatalog = workbookCatalogForClass(getProgressClassId()).filter((item) => item.active !== false && item.subject === subject);
  const scopedCatalog = catalog.length ? catalog : fallbackCatalog;
  const workbooks = uniqueSorted(scopedCatalog.map((item) => item.workbook));
  workbookSelect.innerHTML = workbooks.map((workbook) => `<option value="${escapeAttribute(workbook)}">${escapeHtml(workbook)}</option>`).join("");
  workbookSelect.value = workbooks[0] || "";
  syncActiveWorkbookSections(key, subject);
}

function syncActiveWorkbookSections(key, subject) {
  const workbook = document.getElementById(`${key}Workbook`)?.value || "";
  const schoolYear = document.getElementById(`${key}SchoolYear`)?.value || "";
  const sectionSelect = document.getElementById(`${key}Section`);
  if (!sectionSelect) return;
  const catalog = workbookCatalogForClass(getProgressClassId())
    .filter((item) => item.active !== false && item.subject === subject && item.workbook === workbook)
    .filter((item) => materialMatchesSchoolYear(item, schoolYear));
  const sections = uniqueInOrder(catalog.map(materialPickerSection));
  const selected = sectionSelect.dataset.selected || "";
  sectionSelect.innerHTML = sections.map((section) => `<option value="${escapeAttribute(section)}" ${selected === section ? "selected" : ""}>${escapeHtml(section)}</option>`).join("");
  if (!sections.includes(sectionSelect.value)) sectionSelect.value = sections[0] || "";
  sectionSelect.dataset.selected = sectionSelect.value || "";
}

async function saveActiveWorkbookMaterial(event, scope, subject, animalId = "", groupId = "") {
  event.preventDefault();
  const classId = getProgressClassId();
  const key = activeWorkbookFormKey(scope, subject, animalId, groupId);
  const workbook = document.getElementById(`${key}Workbook`)?.value || "";
  const section = document.getElementById(`${key}Section`)?.value || "";
  const schoolYear = document.getElementById(`${key}SchoolYear`)?.value || activeClassSchoolYear(classId);
  if (!workbook) {
    globalMessage = "Bitte wähle ein Material aus.";
    render();
    return;
  }
  const timestamp = nowIso();
  const existing = (state.activeWorkbookMaterials || []).find((item) => (
    item.classId === classId
    && item.scope === scope
    && item.subject === subject
    && (scope !== "animal" || item.animalId === animalId)
    && (scope !== "group" || item.groupId === groupId)
  ));
  const nextSetting = {
    ...(existing || {}),
    id: existing?.id || makeId(),
    ...
    schoolYear,
    scope,
    animalId: scope === "animal" ? animalId : "",
    groupId: scope === "group" ? groupId : "",
    workbook,
    section,
    validFrom: document.getElementById(`${key}From`)?.value || "",
    validTo: document.getElementById(`${key}To`)?.value || "",
    active: true,
    createdAt: existing?.createdAt || timestamp,
    updatedAt: timestamp
  };
  const activeWorkbookMaterials = existing
    ? (state.activeWorkbookMaterials || []).map((item) => item.id === existing.id ? nextSetting : item)
    : [...(state.activeWorkbookMaterials || []), nextSetting];
  globalMessage = "Aktives Arbeitsmaterial wurde gespeichert.";
  await persistAndRender({ ...state, activeWorkbookMaterials });
}

async function saveActiveClassSchoolYear(event) {
  event.preventDefault();
  const classId = getProgressClassId();
  const activeSchoolYear = normalizeSchoolYear(document.querySelector("#activeClassSchoolYear")?.value || "");
  const classes = state.classes.map((item) => item.id === classId ? { ...item, activeSchoolYear } : item);
  globalMessage = "Aktives Schuljahr der Klasse wurde gespeichert.";
  await persistAndRender({ ...state, classes });
}

async function clearActiveWorkbookMaterial(scope, subject, animalId = "", groupId = "") {
  const timestamp = nowIso();
  const classId = getProgressClassId();
  const activeWorkbookMaterials = (state.activeWorkbookMaterials || []).map((item) => (
    item.classId === classId
    && item.scope === scope
    && item.subject === subject
    && (scope !== "animal" || item.animalId === animalId)
    && (scope !== "group" || item.groupId === groupId)
  ) ? { ...item, active: false, updatedAt: timestamp } : item);
  globalMessage = "Abweichung wurde entfernt.";
  await persistAndRender({ ...state, activeWorkbookMaterials });
}
...
function renderWorkbookCatalogManager() {
  ...
  return `
    <section class="panel">
      ...
      <p class="message">Einträge mit nur einer Startseite werden automatisch bis zur Seite vor dem nächsten Thema ergänzt. Über „bearbeiten“ kannst du Startseite, Endseite und Anzeige jederzeit korrigieren.</p>
      ${renderWorkbookCatalogOverview(items)}
      <form class="weekly-catalog-form" onsubmit="addWorkbookCatalogItem(event)">
        <label class="field">Schuljahr / Klassenstufe
          <select class="select-input" id="catalogSchoolYear">
            ${SCHOOL_YEAR_OPTIONS.map(([value, label]) => `<option value="${escapeAttribute(value)}" ${(activeClassSchoolYear(state.activeClassId) || "none") === value ? "selected" : ""}>${escapeHtml(label)}</option>`).join("")}
          </select>
        </label>
        <label class="field">Fach
      ...
      <div class="table-scroll">
        <table>
          <thead><tr><th>Schuljahr</th><th>Fach</th><th>Lehrwerk</th><th>Teil</th><th>Bereich</th><th>Art</th><th>Seitenbereich</th><th>Thema</th><th>Aktion</th></tr></thead>
          <tbody>
            ${items.map((item) => `
              <tr>
                <td>${escapeHtml(schoolYearLabel(item.schoolYear))}</td>
                ...
                <td>
                  <button class="small-button" type="button" onclick="editWorkbookCatalogItem('${item.id}')">bearbeiten</button>
                  <button class="danger small-button" type="button" onclick="deleteWorkbookCatalogItem('${item.id}')">löschen</button>
                </td>
              </tr>
            `).join("") || `<tr><td colspan="9">Noch keine Einträge vorhanden.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
async function addWorkbookCatalogItem(event) {
  event.preventDefault();
  const schoolYear = normalizeSchoolYear(document.querySelector("#catalogSchoolYear")?.value || "none");
  ...
  const timestamp = nowIso();
  const pageLabel = pageEnd >= page ? formatCatalogPageLabel(page, pageEnd) : String(page);
  const catalogBase = { subject, schoolYear, workbook, part, area, category, title, startPage: page };
  await persistAndRender({
    ...state,
    workbookCatalog: [...(state.workbookCatalog || []), {
      id: makeId(),
      classId: state.activeClassId,
      catalogKey: makeWorkbookCatalogKey(catalogBase),
      subject,
      schoolYear,
      workbook,
      part,
      ...
      pageRangeMode: pageEnd >= page ? "explicit" : "auto",
      title,
      competence,
      note,
      active: true,
...
async function editWorkbookCatalogItem(itemId) {
  const item = workbookCatalogForActiveClass().find((entry) => entry.id === itemId);
  if (!item) return;
  const startAnswer = window.prompt("Startseite:", String(item.startPage || item.page || ""));
  if (startAnswer == null) return;
  const startPage = Number(String(startAnswer).replace(/[^0-9]/g, ""));
  if (!startPage) {
    globalMessage = "Bitte gib eine gültige Startseite ein.";
    render();
    return;
  }
  const endAnswer = window.prompt("Endseite:", String(item.endPage || item.pageEnd || startPage));
  if (endAnswer == null) return;
  const endPage = Number(String(endAnswer).replace(/[^0-9]/g, "")) || startPage;
  if (endPage < startPage) {
    globalMessage = "Die Endseite darf nicht vor der Startseite liegen.";
    render();
    return;
  }
  const displayAnswer = window.prompt("Seitenanzeige:", pageRangeLabel(item));
  if (displayAnswer == null) return;
  const pageLabel = normalizeCatalogPageLabel(displayAnswer) || formatCatalogPageLabel(startPage, endPage);
  const title = window.prompt("Thema / kurzer Inhalt:", item.title || "");
  if (title == null) return;
  const area = window.prompt("Bereich / Oberthema:", item.area || "");
  if (area == null) return;
  const schoolYearAnswer = window.prompt("Schuljahr / Klassenstufe (1, 2, 3, 4, schuljahrübergreifend, ohne Zuordnung):", schoolYearLabel(item.schoolYear));
  if (schoolYearAnswer == null) return;
  const schoolYear = normalizeSchoolYear(schoolYearAnswer);
  const timestamp = nowIso();
  const workbookCatalog = (state.workbookCatalog || []).map((entry) => entry.id === itemId ? {
    ...entry,
    schoolYear,
    area: area.trim(),
    title: title.trim(),
  ...
  globalMessage = "Der Materialeintrag wurde angepasst.";
  await persistAndRender({ ...state, workbookCatalog });
}
...
function focusWorkbookCatalogForm(subject) {
  ...
  const yearSelect = document.querySelector("#catalogSchoolYear");
  if (yearSelect) yearSelect.value = activeClassSchoolYear(state.activeClassId) || "none";
...
function renderClasses() {
  ...
  return `
    ...
    <section class="panel">
      <h2>Tiergruppen bilden</h2>
      <p class="message">Gruppen sind Auswahlhilfen für Wochenpläne und Zuweisungen. Gespeichert werden weiterhin die stabilen Tier-IDs.</p>
      ${groups.map((group) => `
        <div class="manage-row">
          <strong>${escapeHtml(group.name)}</strong>
          <span>${group.animalIds.map((id) => {
            const animal = animals.find((item) => item.id === id);
            return animal ? `${animal.tierEmoji} ${animal.tierName}` : "";
          }).filter(Boolean).map(escapeHtml).join(", ") || "keine Tiere"}</span>
          <button class="danger" type="button" onclick="deleteAnimalGroup('${escapeAttribute(group.id)}')">Gruppe löschen</button>
        </div>
      `).join("") || `<p class="message">Noch keine Tiergruppe angelegt.</p>`}
      <form class="inline-form" onsubmit="addAnimalGroup(event)">
        <label class="field">Gruppenname<input class="text-input" id="newAnimalGroupName" placeholder="z. B. Leseteam"></label>
        <div class="animal-checkbox-grid full-width">
          ${animals.map((animal) => `<label class="check-row"><input type="checkbox" name="newAnimalGroupMember" value="${escapeAttribute(animal.id)}"> ${escapeHtml(animal.tierEmoji)} ${escapeHtml(animal.tierName)}</label>`).join("")}
        </div>
        <button class="primary" type="submit">Gruppe anlegen</button>
      </form>
    </section>
  `;
}

async function addAnimalGroup(event) {
  event.preventDefault();
  const name = document.querySelector("#newAnimalGroupName")?.value.trim() || "";
  const animalIds = [...document.querySelectorAll("input[name='newAnimalGroupMember']:checked")].map((input) => input.value);
  if (!name || !animalIds.length) {
    globalMessage = "Bitte gib einen Gruppennamen ein und wähle mindestens ein Tier.";
    render();
    return;
  }
  const timestamp = nowIso();
  const group = { id: makeId(), classId: state.activeClassId, name, animalIds, createdAt: timestamp, updatedAt: timestamp };
  await persistAndRender({ ...state, animalGroups: [...(state.animalGroups || []), group] });
}

async function deleteAnimalGroup(groupId) {
  if (!confirm("Diese Tiergruppe löschen? Tiere und Lernstände bleiben erhalten.")) return;
  await persistAndRender({ ...state, animalGroups: (state.animalGroups || []).filter((group) => group.id !== groupId) });
}
...
function renderChildrenRegistry() {
  const animals = animalsForActiveClass();
  return `
    <section class="panel">
      <h2>Tiere verwalten</h2>
      <p class="message">Tier-IDs und bestehende QR-Codes bleiben bei Änderungen erhalten. Vornamen und interne Notizen sind nur im geschützten Lehrkraftbereich sichtbar.</p>
      ${animals.map((animal) => `
        <div class="editor-row">
          ...
          <input class="text-input" value="${escapeAttribute(animal.progressNote || "")}" aria-label="Interne Notiz" placeholder="interne Notiz optional" onchange="updateAnimal('${animal.id}', 'progressNote', this.value)">
...
function renderBackup() {
  ...
  return `
    <section class="panel">
      ...
      <p class="privacy-text">Beim normalen Speichern wird keine Datei heruntergeladen. Ein Backup ist eine bewusste Sicherungsdatei.</p>
      <div class="backup-actions">
  ...
  return `
    <div class="merge-report">
      ...
      <div class="status-grid">
        ...
        <div>neu ergänzte aktive Arbeitsmaterialien</div><strong>${lastMergeReport.addedActiveWorkbookMaterials || 0}</strong>
        ...
        <div>übersprungene doppelte aktive Arbeitsmaterialien</div><strong>${lastMergeReport.skippedDuplicateActiveWorkbookMaterials || 0}</strong>
  ...
  return `
    <section class="panel">
      ...
      <div class="status-grid">
        ...
        <div>Speicherstatus</div><strong>${hasUnsavedChanges ? "Nicht gespeicherte Änderungen vorhanden" : "Alle Änderungen gespeichert"}</strong>
        <div>Datenversion</div><strong>${escapeHtml(state.dataVersion || "1.0")}</strong>
        ...
        <div>Anzahl Tiergruppen</div><strong>${(state.animalGroups || []).length}</strong>
        ...
        <div>Anzahl aktiver Arbeitsmaterial-Einstellungen</div><strong>${(state.activeWorkbookMaterials || []).filter((item) => item.active !== false).length}</strong>
      ...
      <p class="privacy-text">Die Daten werden lokal auf diesem iPad/in diesem Browser gespeichert. Beim Klick auf „Daten speichern“ wird keine Datei heruntergeladen. Backups werden nur bewusst im Bereich „Backup / Wiederherstellung“ erstellt.</p>
    </section>
  `;
}
...
async function saveCurrentData() {
  await persist(state);
  globalMessage = "Daten wurden gespeichert.";
  render();
}
...
function pageRangeLabel(item) {
  ...
  if (item.displayPages) return String(item.displayPages).startsWith("S.") ? item.displayPages : `S. ${item.displayPages}`;
  if (item.pageLabel) return formatCatalogDisplayPages(item.pageLabel);
  const start = Number(item.page);
  const end = Number(item.pageEnd);
  return end && end > start ? `S. ${start}–${end}` : `S. ${start}`;
}

function abc1CatalogBookType(item) {
  if (!item || item.workbook !== "ABC der Tiere 1") return "";
  return item.bookType
    || (String(item.part || "").includes("Schreiblehrgang") || String(item.area || "").includes("Schreiblehrgang") ? "Schreiblehrgang" : "")
    || (String(item.part || "").includes("Arbeitsheft") || String(item.area || "").includes("Arbeitsheft") ? "Arbeitsheft" : "")
    || item.area
    || item.category
    || "";
}

function abc1CatalogPartLabel(item) {
  if (!item) return "";
  const part = String(item.part || "").trim();
  const bookType = abc1CatalogBookType(item);
  if (!part) return bookType;
  if (bookType && part.startsWith(bookType)) return part;
  return [bookType, part].filter(Boolean).join(" ");
}

function abc1CatalogFocus(item) {
  if (!item || item.workbook !== "ABC der Tiere 1") return "";
  if (item.focus) return item.focus;
  if (abc1CatalogBookType(item) === "Schreiblehrgang") return item.title || item.topicTitle || "";
  return "";
}

function abc1CatalogAnchorImage(item) {
  if (!item || item.workbook !== "ABC der Tiere 1") return "";
  if (item.anchorImage) return item.anchorImage;
  return extractLabeledText(item.competence, "Ankerbild");
}

function abc1CatalogFibelPages(item) {
  if (!item || item.workbook !== "ABC der Tiere 1") return "";
  if (item.fibelPages) return item.fibelPages;
  return extractLabeledText(item.competence, "zu Fibelseite");
}

function abc1CatalogFibelLabel(item) {
  const pages = abc1CatalogFibelPages(item);
  return pages ? `zu Fibelseite: ${pages}` : "";
}

function abc1CatalogDisplayTitle(item) {
  if (!item || item.workbook !== "ABC der Tiere 1") return "";
  if (item.displayTitle) return item.displayTitle;
  const bookType = abc1CatalogBookType(item);
  const topicTitle = item.topicTitle || item.title || "";
  const focus = abc1CatalogFocus(item);
  const anchorImage = abc1CatalogAnchorImage(item);
  if (bookType === "Schreiblehrgang") return [focus || topicTitle, anchorImage].filter(Boolean).join(" – ");
  return focus && focus !== topicTitle ? `${topicTitle} – ${focus}` : topicTitle;
}

function abc1CatalogBookLine(item) {
  return abc1CatalogPartLabel(item);
}
...
function workbookCatalogFullLabel(item) {
  ...
  if (item.workbook === "ABC der Tiere 1") {
    const bookType = abc1CatalogBookType(item);
    const isWorkbook = bookType === "Arbeitsheft";
    return [
      [item.workbook, abc1CatalogBookLine(item)].filter(Boolean).join(" · "),
      isWorkbook ? item.topicTitle || item.title : abc1CatalogDisplayTitle(item),
      isWorkbook && abc1CatalogFocus(item) ? `Einführung: ${abc1CatalogFocus(item)}` : "",
      pageLabel,
      abc1CatalogFibelLabel(item)
    ].filter(Boolean).join(" – ");
  }
...
function linkWeeklyStatusToProgress(currentState, statusId, options = {}) {
  ...
  const completedPages = normalizeSimpleWorkStatus(status.status) === "fertig"
    ? pages.map(String)
    : (status.completedPages || []).map(String);
  let nextState = upsertWorkbookProgressEntry(currentState, {
  ...
  const progressEntry = findWorkbookProgressDuplicate(nextState.entries || [], status.classId, animal.id, catalog);
  const weeklyPlanStatuses = (nextState.weeklyPlanStatuses || []).map((item) => item.id === status.id ? {
    ...item,
  ...
  return { ...nextState, weeklyPlanStatuses };
}

function removeWeeklyProgressLink(currentState, statusId) {
  const status = (currentState.weeklyPlanStatuses || []).find((item) => item.id === statusId);
  if (!status) return currentState;
  const entries = (currentState.entries || []).map((entry) => {
    if (entry.id !== status.progressEntryId) return entry;
    const sources = String(entry.source || "").split(/[,+]/).map((item) => item.trim()).filter((item) => item && item !== "Wochenplan");
    return {
      ...entry,
      weeklyPlanId: "",
      weeklyPlanDay: "",
...
function findWorkbookCatalogMatchForChildReport(subject, materialFamily, pageText, animalId = "") {
  ...
  const contextIds = workbookCatalogContextIdsForAnimal(animalId, subject);
  const contextualCandidates = candidates.filter((item) => contextIds.has(item.id));
  if (contextualCandidates.length) return contextualCandidates[0];

  const latest = animalId ? latestWorkbookEntry(state.activeClassId, animalId, subject) : null;
  if (latest) {
    const recentCandidates = candidates.filter((item) => (
      item.workbook === latest.materialName
      && (!latest.workbookPart || item.part === latest.workbookPart)
    ));
    if (recentCandidates.length) return recentCandidates[0];
  }

  const locations = new Set(candidates.map((item) => `${item.workbook}|${item.part || ""}`));
  return locations.size === 1 ? candidates[0] : null;
}

function workbookCatalogContextIdsForAnimal(animalId, subject) {
  const ids = new Set();
  if (!animalId) return ids;
  activeWorkbookCatalogIdSet(subject, { animalId, classId: state.activeClassId }).forEach((id) => ids.add(id));
  (state.workbookAssignments || [])
    .filter((assignment) => (
      assignment.classId === state.activeClassId
      && assignment.subject === subject
      && assignment.active !== false
      && (assignment.assignmentMode === "all" || !assignment.animalIds?.length || assignment.animalIds.includes(animalId))
    ))
    .forEach((assignment) => ids.add(assignment.workbookCatalogId));
  weeklyPlansForAnimal(animalId).forEach((plan) => {
    WEEK_DAYS.forEach((day) => {
      weeklyPlanItemsForDay(plan, day, animalId)
        .filter((item) => item.catalogItem?.subject === subject)
        .forEach((item) => ids.add(item.workbookCatalogId));
    });
  });
  return ids;
}
...
function progressEntryTopicLabel(entry) {
  if (!entry) return "–";
  if (entry.materialName === "ABC der Tiere 1") {
    const bookType = entry.bookType || (String(entry.workbookPart || "").includes("Schreiblehrgang") ? "Schreiblehrgang" : String(entry.workbookPart || "").includes("Arbeitsheft") ? "Arbeitsheft" : "");
    const title = entry.displayTitle
      || (bookType === "Schreiblehrgang"
        ? [entry.focus || entry.topic || entry.zusatzText, entry.anchorImage].filter(Boolean).join(" – ")
        : entry.focus && entry.focus !== (entry.topicTitle || entry.topic || entry.zusatzText)
          ? `${entry.topicTitle || entry.topic || entry.zusatzText} – ${entry.focus}`
          : entry.topicTitle || entry.topic || entry.zusatzText);
    const fibel = entry.fibelPages ? `zu Fibelseite: ${entry.fibelPages}` : "";
    return [title, fibel].filter(Boolean).join(" · ") || "–";
  }
  return entry.topic || entry.zusatzText || "–";
}
...
async function saveChildViewSettings(event) {
  ...
  const nextSettings = {
    ...
    abcVisibility: document.querySelector("#childSettingAbcVisibility")?.value || "assigned",
...
async function confirmChildWorkbookReport(reportId, forcedStatus = "", pageMode = "reported") {
  ...
  const catalog = workbookCatalogForClass(report.classId || state.activeClassId).find((item) => item.id === selectedId);
  const animal = state.animals.find((item) => item.id === report.animalId && item.classId === (report.classId || state.activeClassId));
  if (!catalog || !animal) {
    globalMessage = "Bitte wähle zuerst eine passende Zuordnung aus.";
    render();
    return;
  }
  const requestedStatus = normalizeSimpleWorkStatus(forcedStatus || report.status || "fertig");
  const reportedPages = pageNumbersFromText(report.pageText);
  const catalogPages = weeklyCatalogPages(catalog);
  const completedPages = pageMode === "catalog"
    ? catalogPages.map(String)
    : (reportedPages.length ? reportedPages.map(String) : catalogPages.map(String));
  const coversWholeCatalog = catalogPages.length > 0
    && catalogPages.every((page) => completedPages.includes(String(page)));
  const status = requestedStatus === "fertig" && !coversWholeCatalog ? "teilweise" : requestedStatus;
  let nextState = upsertWorkbookProgressEntry(state, {
    classId: report.classId || state.activeClassId,
    animal,
    catalog,
  ...
  const linked = findWorkbookProgressDuplicate(nextState.entries || [], report.classId || state.activeClassId, animal.id, catalog);
  const timestamp = nowIso();
  nextState = {
    ...nextState,
...
async function editChildWorkbookReport(reportId) {
  const report = (state.childWorkbookReports || []).find((item) => item.id === reportId);
  if (!report || report.reviewStatus !== "wartet") return;
  const pageText = window.prompt("Seite oder Seitenbereich korrigieren:", report.pageText || "");
  if (pageText == null) return;
  const normalizedPages = normalizePageText(pageText);
  if (!normalizedPages) {
    globalMessage = "Bitte gib eine gültige Seite oder einen Seitenbereich ein.";
    render();
    return;
  }
  const note = window.prompt("Kurze sachliche Bemerkung ergänzen:", report.note || "");
  if (note == null) return;
  const suggested = findWorkbookCatalogMatchForChildReport(report.subject, report.materialFamily, normalizedPages, report.animalId);
  const reports = (state.childWorkbookReports || []).map((item) => item.id === reportId ? {
    ...item,
    pageText: normalizedPages,
    note: note.trim(),
    suggestedWorkbookCatalogId: suggested?.id || "",
    selectedWorkbookCatalogId: suggested?.id || "",
    updatedAt: nowIso()
  } : item);
  globalMessage = "Kindmeldung wurde angepasst und wartet weiter auf Bestätigung.";
  await persistAndRender({ ...state, childWorkbookReports: reports });
}
...
function parsePageSelection(text, catalog) {
  ...
  return pageNumbersFromText(text)
    .map(String)
    .filter((page) => !allowed.size || allowed.has(page));
}
...
function findWorkbookProgressDuplicate(entries, classId, animalId, catalog) {
  ...
  const byCatalogId = (entries || []).find((entry) => (
  ...
  return (entries || []).find((entry) => (
    ...
    && entry.fach === catalog.subject
    && entry.materialName === catalog.workbook
    && (!entry.workbookPart || !catalog.part || entry.workbookPart === catalog.part)
    && (!entry.workbookArea || !catalog.area || entry.workbookArea === catalog.area)
...
function upsertWorkbookProgressEntry(currentState, { classId, animal, catalog, status, source, weeklyPlan = null, weeklyStatus = null, completedPages = [], note = "" }) {
  const timestamp = nowIso();
  const pages = weeklyCatalogPages(catalog);
  const requestedStatus = normalizeSimpleWorkStatus(status);
  const incomingCompletedPages = requestedStatus === "fertig" && !(completedPages || []).length
    ? pages.map(String)
    : (completedPages || []).map(String);
  const existing = findWorkbookProgressDuplicate(currentState.entries || [], classId, animal.id, catalog);
  const mergedCompletedPages = [...new Set([
    ...((existing?.completedPages || []).map(String)),
    ...incomingCompletedPages
  ])]
    .filter((page) => pages.includes(Number(page)))
    .sort((a, b) => Number(a) - Number(b));
  const completedNumbers = mergedCompletedPages.map((page) => Number(page)).filter(Boolean);
  ...
  const normalizedStatus = requestedStatus === "fertig" && pages.some((page) => !mergedCompletedPages.includes(String(page)))
    ? "teilweise"
    : requestedStatus;
  const pageLabel = catalog.pageLabel || (Number(catalog.pageEnd) > Number(catalog.page) ? `${catalog.page}-${catalog.pageEnd}` : String(catalog.page || targetPage));
  const progressEntry = {
    ...(duplicate || {}),
    ...
    bookType: catalog.bookType || "",
    topicTitle: catalog.topicTitle || catalog.title || "",
    focus: catalog.focus || "",
    anchorImage: catalog.anchorImage || "",
    fibelPages: catalog.fibelPages || "",
    ...
    catalogStartPage: Number(catalog.startPage || catalog.page || targetPage),
    catalogEndPage: Number(catalog.endPage || catalog.pageEnd || catalog.page || targetPage),
    catalogPages: pageLabel,
    pages: pageLabel,
    topic: catalog.displayTitle || catalog.title || catalog.area || "",
    progressNote: note || duplicate?.progressNote || "",
    zusatzText: note || duplicate?.zusatzText || catalog.displayTitle || catalog.title || catalog.area || "",
    status: "fertig",
    workStatus: normalizedStatus,
    completedPages: mergedCompletedPages,
    openPages: pages.map(String).filter((page) => !mergedCompletedPages.includes(page)),
    erledigt: false,
    datumUhrzeit: timestamp,
    source: [...sourceSet].join(" + "),
    confirmationStatus: sourceLabel === "Kind gemeldet" ? "von Lehrkraft bestätigt" : "von Lehrkraft erfasst",
    confirmedAt: sourceLabel === "Kind gemeldet" ? timestamp : duplicate?.confirmedAt || "",
